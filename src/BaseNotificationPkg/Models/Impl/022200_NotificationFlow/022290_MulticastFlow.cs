...
namespace Cozyupk.Shadow.Flow.BaseNotificationPkg.Models.Impl.NotificationFlow
{
    ...
    /// Allows dynamic attachment and detachment of receivers.
    /// </summary>
    public class MulticastFlow<TTarget> : IMulticastNotifier<TTarget>
    {
        /// <summary>
        ...
        /// </summary>
        private ConcurrentDictionary<IUnicastNotifier<TTarget>, byte> Receivers { get; } = new();

        /// <summary>
        /// Adapts or transforms the source notification before it is sent to receivers.
        /// Override this method to implement custom adaptation logic.
        /// </summary>
        protected virtual TTarget OnNotifying(TTarget source)
        {
            // Default transformation logic; override as needed.
            return source;
        }
        ...
        protected virtual bool IsNotifiable(IUnicastNotifier<TTarget> receiver, TTarget output)
        {
            return true;
        }
        ...
        protected virtual void OnExceptionInNotification(IUnicastNotifier<TTarget> receiver, TTarget output, Exception ex)
        {
        ...
        protected internal IEnumerable<IUnicastNotifier<TTarget>> GetLatestReceiversSnapshot()
        {
            List<IUnicastNotifier<TTarget>> snapshot = new();
        ...
        public void RegisterSendingFlow(INotificationHandler<TTarget> flow, Action<Action>? runner = null)
        {
            if (flow == null)
            ...
            flow.RegisterReceivingHandler(source =>
            {
                // Adapt the source to the target type
                var output = OnNotifying(source);
        ...
        public void AttachReceivingFlow(params IUnicastNotifier<TTarget>[] flow)
        {
            lock (ReceiversLock)
            {
                if (flow == null)
                    throw new ArgumentNullException(nameof(flow), "Flow cannot be null.");
        ...
        public void DetachReceivingFlow(params IUnicastNotifier<TTarget>[] flow)
        {
            lock (ReceiversLock)
            {
                foreach (var receiver in flow)
