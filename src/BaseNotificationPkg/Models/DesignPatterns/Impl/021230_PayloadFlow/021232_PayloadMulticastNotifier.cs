...
namespace Cozyupk.Shadow.Flow.BaseNotificationPkg.Models.Impl.PayloadFlow
{
        ...
        where TPayloadMeta : class
    {
        ...
        private ConcurrentDictionary<ISenderPayloadConsumer<TSenderMeta, TPayloadMeta, TPayloadBody>, byte> Consumers { get; } = new();
        ...
        /// </summary>
        private IEnumerable<ISenderPayloadConsumer<TSenderMeta, TPayloadMeta, TPayloadBody>> GetLatestConsumersCppy()
        {
            ...
            {
                consumersCopy = new List<ISenderPayloadConsumer<TSenderMeta, TPayloadMeta, TPayloadBody>>();
            ...
            notificationHandler.Handle = payload =>
            {
                ...
                runner(() =>
                {
                    foreach (var consumer in consumersCopy)
                    {
                        ...
                        consumer.SenderPayloadArrivalNotifier.Notify(senderPayload);
                    }
                });
            };
        }
        ...
        public void RemoveConsumer(ISenderPayloadConsumer<TSenderMeta, TPayloadMeta, TPayloadBody> consumer)
        {
            Consumers.TryRemove(consumer, out _);
        }
        ...
        /// </summary>
        protected internal IEnumerable<ISenderPayloadConsumer<TSenderMeta, TPayloadMeta, TPayloadBody>> GetConsumers()
        {
